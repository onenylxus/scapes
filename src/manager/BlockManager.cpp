...
std::vector<float> BlockManager::solidVertexData = BlockManager::LoadVertexData("res/blocks/solid_vertex_data.txt");
std::vector<float> BlockManager::liquidVertexData = BlockManager::LoadVertexData("res/blocks/liquid_vertex_data.txt");
std::vector<float> BlockManager::gaseousVertexData = BlockManager::LoadVertexData("res/blocks/gaseous_vertex_data.txt");
std::vector<Block::Data> BlockManager::blocks;

// Init block manager
void BlockManager::Init()
{
	Logger::LogTrace(ModuleData::Name::BLOCK_MANAGER, "void Init()");
...
{
	Logger::LogTrace(ModuleData::Name::BLOCK_MANAGER, "void Destroy()");
...
std::vector<float> BlockManager::LoadVertexData(const char *path)
{
	Logger::LogTrace(ModuleData::Name::BLOCK_MANAGER, "std::vector<float> LoadVertexData(const char* path)");

	std::vector<float> vertexData;
	FILE *file = fopen(path, "r");
	if (file == nullptr)
	{
		Logger::LogError(ModuleData::Name::BLOCK_MANAGER, "Failed to open vertex data file: %s", path);
		return vertexData;
	}
	...
	if (vertexData.empty())
	{
		Logger::LogError(ModuleData::Name::BLOCK_MANAGER, "Vertex data file is empty: %s", path);
		return vertexData;
	}
	if (vertexData.size() != 144)
	{
		Logger::LogError(ModuleData::Name::BLOCK_MANAGER, "Vertex data file is malformed: %s %d", path, vertexData.size());
		return vertexData;
	}
	Logger::LogInfo(ModuleData::Name::BLOCK_MANAGER, "Loaded vertex data from file: %s", path);
	return vertexData;
}
...
{
	Logger::LogTrace(ModuleData::Name::BLOCK_MANAGER, "void GetMeshFace(const Block::Type &type, const Block::Face &face, const unsigned int &corner, const glm::vec3 &position, std::vector<float> &vertices)");
...
{
	Logger::LogTrace(ModuleData::Name::BLOCK_MANAGER, "int GetMeshFaceSize(const Block::Type &type)");
